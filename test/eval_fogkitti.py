...
from datasets import FogKITTI, Collate
...
if __name__ == "__main__":
    ...
    with open("./datasets/fog-kitti.yaml") as stream:
    ...
    for it, batch in enumerate(
        ...
        if id_vote == args.num_votes:
            ...
            label_file = label_file.replace("fog_velodyne", "predictions")[:-3] + "label"
            
            label_file = os.path.join(args.result_folder, label_file.lstrip('/'))
            print("label_files is: ", label_file)
            ...
            # true_label_path = absolute_path.replace("code/WaffleIron/AMW-NET/result/semantic_fogkitti/predictions_fogkitti_32_9", "sda/Dataset/Weather_KITTI/Fog-KITTI").replace("predictions", "fog_label")
