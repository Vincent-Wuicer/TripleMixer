...
from datasets import RainKITTI, Collate
...
if __name__ == "__main__":
    ...
    with open("./datasets/rain-kitti.yaml") as stream:
    ...
    for it, batch in enumerate(
        ...
        if id_vote == args.num_votes:
            ...
            # print("filename 0 is: ", batch["filename"][0])
            # print("filename -1 is: ", batch["filename"][-1])
            # print("rootdir: ", dataset.rootdir)
            ...
            label_file = label_file.replace("rain_velodyne", "predictions")[:-3] + "label"
            
            label_files = os.path.join(args.result_folder, label_file.lstrip('/'))
            
            # print("result_folder is: ", args.result_folder)
            # print("label_files is: ", label_files)
            # print("split is: ", os.path.split(label_files)[0])

            os.makedirs(os.path.split(label_files)[0], exist_ok=True)
            
            label.tofile(label_files)
