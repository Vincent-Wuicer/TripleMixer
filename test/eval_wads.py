...
from datasets import SnowWads, Collate
...
if __name__ == "__main__":
    ...
    with open("./datasets/snow-wads.yaml") as stream:
    ...
    for it, batch in enumerate(
        ...
        # print("labels 0 is: ", torch.sum(labels == 0).item())  
        # print("labels 1 is: ", torch.sum(labels == 1).item())  
        # print("labels 9 is: ", torch.sum(labels == 9).item())
        
        net_inputs = (feat, cell_ind, occupied_cell, neighbors_emb)
        #print("net_inputs shape is: ", net_inputs[2].shape)
            ...
            with torch.inference_mode():
                ...
                for b in range(out.shape[0]):
                    ...
                    if vote is None:
                        ...
                        print("temp shape after softmax is: ", torch.softmax(temp, dim=1).shape)
                        vote += torch.softmax(temp, dim=1)
        id_vote += 1
        ...
        if id_vote == args.num_votes:
            ...
            print("label is 0: ", np.sum(label == 0))
            print("label is 0:", np.sum(label == 1))
            print("label is 0:", np.sum(label == 9))
