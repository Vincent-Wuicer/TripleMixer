...
if __name__ == '__main__':
    ...
    betas = 0.05  # [0.008, 0.05, 0.2]
    available_alpha = 0.005  # [0.005, 0.01, 0.02, 0.03, 0.06]
    
    parameter_set = ParameterSet(alpha=available_alpha, gamma=0.000001, beta=betas)  # beta = [0.008, 0.05, 0.2]

    ori_point_path = '/home/hit/sda/Dataset/Ori_KITTI/sequences/06/velodyne/000105.bin'
    dst_folder = '/home/hit/sda/Simu_tool/fog_kitti/06_05_03'

    points = np.fromfile(ori_point_path, dtype=np.float32)
    ...
    lidar_save_path = os.path.join(dst_folder,'fog_velodyne', ori_point_path.split('/')[-1])
    ...
    semantic_label_save_path = os.path.join(dst_folder,'fog_labels', ori_point_path.split('/')[-1].replace('.bin', '.label'))
