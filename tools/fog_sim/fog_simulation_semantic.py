...
def P_R_fog_soft(p: ParameterSet, pc: np.ndarray, label: np.ndarray, original_intesity: np.ndarray, noise: int, gain: bool = False,
    ...
    fog_label = np.zeros(len(pc), dtype=int)
    ...
    assert len(fog_label) == len(aug_label)

    return augmented_pc, simulated_fog_pc, num_fog, info_dict, fog_label, aug_label
...
def simulate_fog(p: ParameterSet, pc: np.ndarray, label:np.ndarray, noise: int, gain: bool = False, noise_variant: str = 'v1',
    ...
    if soft:
        augmented_pc, simulated_fog_pc, num_fog, info_dict, fog_label, aug_label = P_R_fog_soft(p, augmented_pc, augmented_label, original_intensity, noise, gain,
                                                                 noise_variant)
...
if __name__ == '__main__':
    ...
    for seq in [str(i).zfill(2) for i in range(22)]:
        ...
        fog_txt = os.path.join(args.root_folder, seq + '/info_fog.txt')
        
        with open(fog_txt, 'r') as f:
            lines = f.readlines()
            inte_folder = lines[0].strip()
            betas = float(lines[1].strip())
            available_alpha = float(lines[2].split('=')[1].strip())
        
        
        print("inte_folder is: ", inte_folder)
        ...
        print("available_alpha is: ", available_alpha)



        def _map(i: int) -> None:
            parameter_set = ParameterSet(alpha=available_alpha, gamma=0.000001, beta=betas)  # beta = [0.008, 0.05, 0.2]
            ...
            points, fog_points, num_fog, _ , fog_label, aug_label= simulate_fog(parameter_set, points, label, 10)
            #print("ori points is: ", points.shape)
            # print("aug points is: ", points.shape)
            # print("fog_points is: ", fog_points.shape)
            # print("num_fog is: ", num_fog)
            #print("fog_label 1 is: ", np.sum(fog_label == 1))
            # print("fog_label 0 is: ", np.sum(fog_label == 0))


            lidar_save_path = os.path.join(dst_folder,'fog_velodyne', all_files[i].split('/')[-1])
            ...
            label_save_path = os.path.join(dst_folder,'fog_label', all_files[i].split('/')[-1].replace('.bin', '.label'))
            ...
            labels = (fog_label).astype(np.int32).tofile(label_save_path)

            semantic_label_save_path = os.path.join(dst_folder,'fog_labels', all_files[i].split('/')[-1].replace('.bin', '.label'))
            if not os.path.exists(os.path.dirname(semantic_label_save_path)):
                os.makedirs(os.path.dirname(semantic_label_save_path), exist_ok=True)
            semantic_labels = (aug_label).astype(np.int32).tofile(semantic_label_save_path)
