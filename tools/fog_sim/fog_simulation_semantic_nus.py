...
if __name__ == '__main__':
    ...
    for idx, (split_files, split_paths) in enumerate(zip(splits_all_files, splits_all_paths)):
        ...
        lidar_save_root = os.path.join(dst_folder, num_str, 'fog_velodyne')
        if not os.path.exists(lidar_save_root):
            os.makedirs(lidar_save_root)

        snow_label_root = os.path.join(dst_folder, num_str, 'fog_label')
        if not os.path.exists(snow_label_root):
            os.makedirs(snow_label_root)

        label_save_root = os.path.join(dst_folder,  num_str, 'fog_labels')
        ...
        def _map(i: int) -> None:
            ...
            if os.path.exists(file_path):
                ...
                points, fog_points, num_fog, _ , fog_label, aug_label= simulate_fog(parameter_set, points, labels, 10)
                ...
                fog_label.astype(np.int32).tofile(label_save_path)
                aug_label.astype(np.int32).tofile(semanticlabel_save_path)
