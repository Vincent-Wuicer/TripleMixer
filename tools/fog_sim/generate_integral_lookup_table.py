...
def parse_arguments():
    ...
    parser.add_argument('-b', '--beta', help='backscattering coefficient', type=float, default=0.002)
    parser.add_argument('-s', '--save_path', help='path to ',
                        default=str(Path(__file__).parent.absolute() / 'integral_lookup_tables_seg_light_0.002beta/original'))
...
def generate_integral_lookup_tables(arguments) -> None:
    ...
    for alpha in arguments.alphas:
        ...
        p = ParameterSet(n=n, r_range=r_0_max, alpha=alpha, beta=arguments.beta)
        print(p.beta * 1)
