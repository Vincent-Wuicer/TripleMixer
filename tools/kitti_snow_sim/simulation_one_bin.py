...
def estimate_laser_parameters(pointcloud_planes, calculated_indicent_angle, power_factor=15, noise_floor=0.7,
    ...
    #print("normalized_intensitites is: ",normalized_intensitites)
    # linear model
    p = None
    stat_values = None
    ...
    if estimation_method == 'linear':
        ...
        #print("relative_output_intensity is: ", relative_output_intensity)
...
def calculate_plane(pointcloud, standart_height=-1.55):
    ...
    # print("pc_rect.shape[0] is: ",pc_rect.shape[0])
    # print("pc_rect.shape[1] is: ",pc_rect.shape[1])
    ...
    if pc_rect.shape[0] <= pc_rect.shape[1]:
        ...
        try:
            reg = RANSACRegressor(loss='squared_loss', max_trials=100).fit(pc_rect[:, [0, 1]], pc_rect[:, 2])
            w = np.zeros(3)
...
def process_single_channel(root_path: str, particle_file_prefix: str, orig_pc: np.ndarray, orig_label: np.ndarray, beam_divergence: float,
    ...
    for j, beam_dict in enumerate(occlusion_list):
        ...
        if channel in [53, 55, 56, 58]:
            ...
            max_intensity = 255
        # 对每个激光束的原始强度进行调整，考虑到焦距的斜率和偏移量。然后用 np.clip 函数确保调整后的强度在0和最大强度之间
        ...
        # 使用雷达方程调整计算调整后的强度，这个计算可能涉及到接收到的功率与距离的平方成反比，以及某个与设备相关的常数 beta_0
        CA_P0 = i_adjusted * d_orig ** 2 / beta_0
        # 如果当前激光束的字典中有多于一个键，则意味着存在遮挡物，很可能是雪花
        if len(beam_dict.keys()) > 1:                           # otherwise there is no snowflake in the current beam
            ...
            # 对最大强度值进行调整，并确保它在允许的范围内
            i_max += max_intensity * focal_slope * np.abs(focal_offset - (1 - d_max/120)**2)
            i_max = np.clip(i_max, min_intensity, max_intensity)
            # 如果计算出的距离与原始距离的差异小于一个阈值，则只更新强度，而不是位置
            if abs(d_max - d_orig) < 2 * (1 / intervals_per_meter):  # only alter intensity  只改变强度
                # 将点的标记设置为 1，表示强度被更改
                pc[j, 4] = 1
                new_i = int(i_max)
                ...
                pc[j, 0] = pc[j, 0] * d_scaling_factor * 2.5
                pc[j, 1] = pc[j, 1] * d_scaling_factor * 2.5
                pc[j, 2] = pc[j, 2] * d_scaling_factor * 2.5
                p_label[j] = 110   # snow class 
...
def augment(pc: np.ndarray, label: np.ndarray, particle_file_prefix: str, beam_divergence: float, shuffle: bool = True,
    ...
    for item in channel_list:
        ...
        snowflakes_sum += sum(p_label.reshape(-1) == 110)
        
    aug_pc[:, 3] = aug_pc[:, 3]

    scattered = aug_pc[:, 4] == 2
    # scattered_count = np.sum(scattered)
    #print("Number of scattered points:", scattered_count)

    above_threshold = aug_pc[:, 3] > relative_output_intensity[:]

    # above_threshold_count = np.sum(above_threshold)
    #print("Number of above_threshold_count points:", above_threshold_count)

    scattered_or_above_threshold = np.logical_or(scattered, above_threshold)
    #scattered_or_above_threshold_count = np.sum(scattered_or_above_threshold)
    #print("Number of scattered_or_above_threshold_count points:", scattered_or_above_threshold_count)

    num_removed = np.logical_not(scattered_or_above_threshold).sum()

    print("num_removed point is: ",num_removed )

    # print("before aug_pc count is: ",aug_pc.shape[0])
    aug_pc = aug_pc[np.where(scattered_or_above_threshold)]
    # print("after aug_pc count is: ",aug_pc.shape[0])
    ...
    #print("num_attenuated point is: ",num_attenuated )

    num_show = (aug_pc[:, 4] == 2).sum()
    print("num_show point is: ",num_show )

    num_clean = (aug_pc[:, 4] == 0).sum()
    #print("num_clean point is: ",num_clean ) 
...
def parse_arguments():
    ...
    parser.add_argument('-t', '--terminal_velocity', help='terminal velocity', type=str,
                        default= '2.0')
    parser.add_argument('-m', '--mode', help='snow mode', type=str,
                        default= 'gunn')
...
def process_file(file_path, args, dst_folder, snowflake_file_prefix, beam_divergence, seq):
    try:
        points = np.fromfile(file_path, dtype=np.float32)
        points = points.reshape((-1, args.n_features))
        assert points is not None
        label_path = file_path.replace('Dataset/Ori_KITTI', 'SemanticKITTI/dataset').replace('velodyne', 'labels')[:-3] + 'label'
        #print("label_path:", label_path)
        label = np.fromfile(file_path.replace('Dataset/Ori_KITTI', 'SemanticKITTI/dataset').replace('velodyne', 'labels')[:-3] + 'label', dtype=np.uint32).reshape((-1, 1))
        label = label & 0xFFFF
        unique_labels = np.unique(label)
        #print("Unique labels:", unique_labels)
        assert label is not None
        # # point_count = points.shape[0]
        # #print("point_count is: ", point_count)
        ...
        #augmented_pointcloud_count = augmented_pointcloud.shape[0]
        #print("augmented_pointcloud_count is: ", augmented_pointcloud_count)
        augmented_pointcloud[:, 3] = augmented_pointcloud[:, 3] / 255
        # 这里给出的理由是默认雪花点不仅是强度发生改变，并且坐标同样发生改变  所以选择最后一维等于2的点
        # 我觉得是合理的，单纯衰减是不能够保证雪花点的  因为强度影响因素很多，会发生误判
        # augmented_pointcloud[:, -1] == 1只是被影响的点并不是雪花点  个人判断
        labels = (augmented_pointcloud[:, -1] == 2).astype(np.int32)
        #print('The count of label',np.count_nonzero(augmented_pointcloud[:, -1] == 2))
        augmented_pointcloud = augmented_pointcloud[:, :4]
        lidar_save_path = os.path.join(dst_folder,seq, 'snow_velodyne', file_path.split('/')[-1])
        ...
        label_save_path = os.path.join(dst_folder,seq, 'snow_label', file_path.split('/')[-1].replace('.bin', '.label'))
        if not os.path.exists(os.path.dirname(label_save_path)):
            os.makedirs(os.path.dirname(label_save_path))
        labels.tofile(label_save_path)

        auglabel_save_path = os.path.join(dst_folder,seq, 'snow_labels', file_path.split('/')[-1].replace('.bin', '.label'))
        if not os.path.exists(os.path.dirname(auglabel_save_path)):
            os.makedirs(os.path.dirname(auglabel_save_path))
        augmented_label = augmented_label.astype(np.int32)
        augmented_label.tofile(auglabel_save_path)
...
if __name__ == '__main__':
    ...
    info_txt = '/home/hit/sda/Dataset/Ori_KITTI/sequences/07/info.txt'

    with open(info_txt, 'r') as f:
        for line in f.readlines():
            parts = line.strip().split(':')
            if len(parts) == 2:
                identifier, value = parts[0].strip().lower(), parts[1].strip()
                if identifier == 'snowfall rate':
                    snowfall_rate = float(value)
                elif identifier == 'terminal velocity':
                    terminal_velocity = float(value)
                elif identifier == 'mode':
                    mode = value

    rain_rate = snowfall_rate_to_rainfall_rate(float(snowfall_rate), float(terminal_velocity))
    #print('The rain_rate is : ' , rain_rate)
    ...
    print("rain_rate is:", rain_rate, "and occupancy is:", occupancy, ' and snowflake_file_prefix is:', snowflake_file_prefix)
    # print('*******************************')

    file = '/home/hit/sda/Dataset/Ori_KITTI/sequences/07/velodyne/000002.bin'
    seq = '07'
    
    # dstseq_folder = os.path.join(dst_folder, seq)
    # with open(dstseq_folder, "w") as txt_file:
    #     txt_file.write(f"snowfall rate\n{snowfall_rate}\n\n")
    #     txt_file.write(f"terminal velocity\n{terminal_velocity}\n\n")
    #     txt_file.write(f"Gunn\n{mode}\n")

    with ProcessPoolExecutor(max_workers=100) as executor:
        futures = {executor.submit(process_file, file, args, dst_folder, snowflake_file_prefix, beam_divergence, seq)}
