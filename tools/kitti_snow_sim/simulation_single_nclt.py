...
def augment(pc: np.ndarray, particle_file_prefix: str, beam_divergence: float, shuffle: bool = True,
    ...
    h = -0.5
    ground = np.logical_and(np.matmul(pc[:, :3], np.asarray(w)) + h < 0,
                            np.matmul(pc[:, :3], np.asarray(w)) + h > -3.5)
    pc_ground = pc[ground]
...
if __name__ == '__main__':
    ...
    lidar_path = '/home/hit/sda/NCLT/1326652770479038.bin'
    pc = load_pc_nclt('/home/hit/sda/NCLT/1326652770479038.bin')

    stats, augmented_pointcloud = augment(pc, only_camera_fov=False,
    ...
    lidar_save_path = '/home/hit/sda/NCLT/1326652770479038_snow.bin'
    ...
    #         lidar_save_path = os.path.join(dst_folder,seq, 'snow_velodyne', all_files[i].split('/')[-1])

    #         if not os.path.exists(os.path.dirname(lidar_save_path)):
    #             os.makedirs(os.path.dirname(lidar_save_path))
    #         augmented_pointcloud.astype(np.float32).tofile(lidar_save_path)

    #         label_save_path = os.path.join(dst_folder,seq, 'snow_labels', all_files[i].split('/')[-1].replace('.bin', '.label'))
