...
def calculate_plane(pointcloud, standart_height=-1.65):
    """
...
def get_occlusions(beam_angles: np.ndarray, ranges_orig: np.ndarray, root_path: str, particle_file: str,
    ...
    # /home/hit/code/nuscenes_snow/npy

    if root_path:
        path = Path(root_path) / 'training' / 'snowflakes' / 'npy' / particle_file
    else:
        path = Path(__file__).parent.parent.parent.absolute() / 'code'/'nuscenes_snow'/'npy' / particle_file
...
def augment(pc: np.ndarray, label: np.ndarray, particle_file_prefix: str, beam_divergence: float, shuffle: bool = True,
    ...
    # 得到scattered_or_above_threshold 为true的索引
    ...
    print("num_removed num is: ",num_removed)
...
if __name__ == '__main__':
    ...
    # gunn   Sekhon
    # light: snowfall_rate = '1.0' , terminal_velocity = '2.0'; 
    # moderate: snowfall_rate = '2.0', terminal_velocity = '1.6'; 
    # heavy: snowfall_rate = '2.5', terminal_velocity = '2.0' 
    ...
    for idx, (split_files, split_paths) in enumerate(zip(splits_all_files, splits_all_paths)):
        ...
        print("snow _flake is: ", snowflake_file_prefix)

        # root
        num_str = str(idx).zfill(2)
        Path(dst_folder).mkdir(parents=True, exist_ok=True)
        lidar_save_root = os.path.join(dst_folder, num_str, 'snow_velodyne')
        if not os.path.exists(lidar_save_root):
            os.makedirs(lidar_save_root)

        snow_label_root = os.path.join(dst_folder, num_str, 'snow_label')
        if not os.path.exists(snow_label_root):
            os.makedirs(snow_label_root)

        label_save_root = os.path.join(dst_folder,  num_str, 'snow_labels')
        ...
        for i,_ in enumerate(split_files):
            ...
            if os.path.exists(file_path):
                points = np.fromfile(file_path, dtype=np.float32, count=-1).reshape([-1, 5])
                ...
                #print("lidar base file is: ", lidar_filename)
                snow_labels = (augmented_pointcloud[:, -1] == 2).astype(np.int32)
                snow_label_save_path = os.path.join(snow_label_root, lidar_filename.replace('.bin', '.label'))
                snow_labels.tofile(snow_label_save_path)
                ...
                label_save_path = os.path.join(label_save_root, lidar_filename.replace('.bin', '.label'))
                #print("label_save_path is:", label_save_path)
                augmented_label.astype(np.int32).tofile(label_save_path)

            else:
                print("No lidar bin found!")
                continue
