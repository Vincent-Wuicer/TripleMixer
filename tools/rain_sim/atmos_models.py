...
## KITTI:     def __init__(self,atm_model,m=1.328,lam=903,rmax=500,rmin=4,bdiv=3e-3,dst=0.05,
                 #dR=0.04,saved_model=False,mode='strongest'):

## ORI: rmax=200,rmin=1.5
...
class LISA():
    ...
    def __init__(self,atm_model,m=1.328,lam=903,rmax=300,rmin=3,bdiv=3e-3,dst=0.05,
                 dR=0.04,saved_model=False,mode='strongest'):
        '''
    ...
    def augment_mc(self,pc,label,Rr):
        '''
        ...
        # print("pc_new shape is: ", pc_new.shape)
        # print("new_label shape is: ", new_label.shape)
        
        leng = len(pc)
        lenglab = len(label)
        # print("len(label) is: ", len(label))
        # print("len(pc) is: ", len(pc))
        ...
        for i in range(leng):
            ...
            if ref!=0:
                #result = self.lisa_mc(x,y,z,aug_label,ref,Rr)
                *point_data, label_value = self.lisa_mc(x,y,z,aug_label,ref,Rr) 
            else:
                #result = self.lisa_mc(x,y,z,aug_label,self.rmax**(-2)*1.2,Rr)
                *point_data, label_value = self.lisa_mc(x,y,z,aug_label,self.rmax**(-2)*1.2,Rr)   
                
            pc_new[i, :5] = point_data
            new_label[i] = label_value
            
        # pc_new[i, :4] = result[:4]  # Assuming the first four values are x, y, z, reflectivity
        # pc_new[i, 4] = result[4]    # The fifth value is the label
        # new_label[i] = result[5]    # The sixth value is the new label
        
        pc_new_filtered = pc_new[pc_new[:,-1] != 0]
        new_label_filtered = new_label[pc_new[:,-1] != 0]
        assert len(new_label_filtered) == len(pc_new_filtered)
        
        return pc_new_filtered, new_label_filtered
    
    
    
    
    def lisa_mc(self,x,y,z, aug_label, ref,Rr):
        '''
        ...
        return x, y, z, ref_new, labl, aug_label
