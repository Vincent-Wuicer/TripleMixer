...
from atmos_models_no_semantic import LISA
import numpy as np
...
if __name__ == '__main__':
    ...
    for seq in [str(i).zfill(2) for i in range(11, 22)]:
        # if seq not in ['00']:
        #     continue
        ...
        def _map(i: int) -> None:
            ...
            rain_points = lisa.augment_mc(points, rain_rate)
            ...
            lidar_save_path = os.path.join(dst_folder,'rain_velodyne', all_files[i].split('/')[-1])
            ...
            # semanticlabel_save_path = os.path.join(dst_folder,'rain_labels', all_files[i].split('/')[-1].replace('.bin', '.label'))
